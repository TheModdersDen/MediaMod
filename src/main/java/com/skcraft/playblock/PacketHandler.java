...
public class PacketHandler {
	...
	private static SPacketUpdateTileEntity spacket;
	private static ByteBuf bBuffer;
	private static PacketBuffer pBuffer = new PacketBuffer(bBuffer);
	private static EntityPlayerMP player;
	private static Packet packet = TileEntityProjector.updatePacket;
    ...
    public static void sendToServer(PlayBlockPayload payload) {
        ...
        FMLProxyPacket packet = new FMLProxyPacket(pBuffer, PlayBlock.CHANNEL_ID);
    ...
    public static void sendToClient(PlayBlockPayload payload, List<EntityPlayer> players) {
        ...
        try {
            payload.write(out);
            player.connection.sendPacket(TileEntityProjector.updatePacket);
            out.flush();
        } catch (IOException e) {
        ...
        if (players == null) {
            SharedRuntime.networkWrapper.sendToAll((FMLProxyPacket) packet);
        } else {
            for (EntityPlayer player : players) {
                if (player instanceof EntityPlayerMP) {
                    SharedRuntime.networkWrapper.sendTo((FMLProxyPacket) packet, (EntityPlayerMP) player);
                }
            }
        }
    }
